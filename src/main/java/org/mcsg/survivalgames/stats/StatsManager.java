...
public class StatsManager {
    ...
    public void addKill(Player p, Player killed, int arenaid, String name){
        ...
        if(kslevel > 3){
        	msgmgr.broadcastFMessage(PrefixType.INFO, "killstreak.level"+((kslevel>5)?5:kslevel), "player-"+p.getDisplayName(), "arenaname-"+name);
        }
        else if(kslevel > 0){
            for (Player pl : GameManager.getInstance().getGame(arenaid).getAllPlayers()) {
            	msgmgr.sendFMessage(PrefixType.INFO, "killstreak.level"+((kslevel>5)?5:kslevel), pl, "player-"+p.getDisplayName(), "arenaname-"+name);
            }
        }
    }
